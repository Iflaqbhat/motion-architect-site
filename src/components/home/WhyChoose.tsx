    ...
    <section className="mx-auto max-w-[110rem] px-6 py-28 md:px-12">
      <Reveal>
        <div className="grid gap-10 border-b border-ink/15 pb-10 md:grid-cols-12">
          <h2 className="text-4xl leading-[1.05] md:col-span-7 md:text-5xl">
            Why choose
            <br />
            <span className="text-outline">Yuva Group</span>
          </h2>
          <p className="self-end text-base font-medium leading-relaxed text-muted-foreground md:col-span-4 md:col-start-9">
        ...
        {WHY_CHOOSE.map((w, i) => (
          <Reveal key={w.title} delay={i * 0.06} className={i % 3 === 1 ? "md:mt-16" : i % 3 === 2 ? "md:mt-32" : ""}>
            <div className="group">
              ...
              <h3 className="mt-4 text-2xl transition-transform duration-500 group-hover:translate-x-2">
                {w.title}
              </h3>
              <span className="mt-4 block h-px w-full bg-ink/15" />
              <p className="mt-4 text-base font-medium leading-relaxed text-muted-foreground">{w.copy}</p>
            </div>
          </Reveal>
        ))}
      </div>
